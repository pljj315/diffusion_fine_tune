        ...
        for lora_file in lora2emd_dict:
          ...
          for img_path, seg_path in img_seg_paths:
            for prom_id, tmp_prompt in enumerate(prompts):
                attn_change_list = ['wo_prod', 'attn', '']
                seed = random.randint(0, 99999999)
                payload['seed'] = seed
                for attn_change in attn_change_list:
                    if attn_change == 'wo_prod':
                        pro_name = ''
                        pro_tmp_prompt = tmp_prompt.replace('商品', pro_name)
                        payload['alwayson_scripts']['controlnet']['args'][0]['inpaint_product_prompt']= pro_name

                    elif attn_change == 'attn':
                        ...
                        pro_name = os.path.split(os.path.splitext(img_path)[0])[-1]
                        pro_tmp_prompt = tmp_prompt.replace('商品', pro_name)
                        payload['alwayson_scripts']['controlnet']['args'][0]['inpaint_product_prompt']= ''
